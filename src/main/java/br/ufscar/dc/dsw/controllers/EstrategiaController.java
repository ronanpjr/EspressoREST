...
public class EstrategiaController {
    ...
    public ResponseEntity<List<EstrategiaDto>> listarEstrategias() {
        ...
        List<EstrategiaDto> dtos = estrategias.stream()
                .map(estrategiaService::convertModelToDto)
                .collect(Collectors.toList());
        return ResponseEntity.ok(dtos);
    }

    @GetMapping("/{id}")
    public ResponseEntity<EstrategiaDto> buscarEstrategiaPorId(@PathVariable UUID id) {
        EstrategiaModel estrategia = estrategiaService.findById(id);
        if (estrategia == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(estrategiaService.convertModelToDto(estrategia));
    }

    @PostMapping
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<EstrategiaDto> criarEstrategia(@RequestPart("estrategia") EstrategiaDto dto,
                                                         @RequestPart(value = "imagensExemplo", required = false) List<MultipartFile> imagensExemplo) {
        EstrategiaModel novaEstrategia = estrategiaService.save(dto, imagensExemplo);
        URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
                .buildAndExpand(novaEstrategia.getId()).toUri();
        return ResponseEntity.created(location).body(estrategiaService.convertModelToDto(novaEstrategia));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<EstrategiaDto> atualizarEstrategia(@PathVariable UUID id,
                                                             @RequestPart("estrategia") EstrategiaDto dto,
                                                             @RequestPart(value = "imagensExemplo", required = false) List<MultipartFile> imagensExemplo) {
        if (estrategiaService.findById(id) == null) {
            return ResponseEntity.notFound().build();
        }
        dto.setId(id);
        EstrategiaModel estrategiaAtualizada = estrategiaService.save(dto, imagensExemplo);
        return ResponseEntity.ok(estrategiaService.convertModelToDto(estrategiaAtualizada));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> deletarEstrategia(@PathVariable UUID id) {
        if (estrategiaService.findById(id) == null) {
            return ResponseEntity.notFound().build();
        }
